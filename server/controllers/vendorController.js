...
const cycleStatus = asyncHandler(async (req, res) => {
  ...
  const vendor = await Vendor.update(req.params.id, req.user.id, { status: next });
  sendSuccess(res, { vendor });
});
...
module.exports = { listVendors, getVendor, createVendor, updateVendor, cycleStatus, deleteVendor };
