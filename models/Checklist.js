...
class Checklist {
  ...
  static async create(
    ...
    const { rows } = await query(
      `INSERT INTO checklist_tasks (user_id, text, category, due_date, sort_order)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [userId, text, category, dueDate || null, sortOrder],
    );
    return rows[0];
  }
  ...
  /** Bulk-insert seed tasks, skipping duplicates by text per user. */
  static async bulkCreate(userId, tasks) {
    const created = [];
    for (const t of tasks) {
      const { rows } = await query(
        `INSERT INTO checklist_tasks (user_id, text, category, due_date)
         VALUES ($1, $2, $3, $4)
