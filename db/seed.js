...
const MUSIC = [
  { section: 'Processional', track_id: '1440830605', track_name: 'A Thousand Years',   artist_name: 'Christina Perri',  artwork_url: null, preview_url: null },
  { section: 'First Dance',  track_id: '1440830606', track_name: 'Perfect',             artist_name: 'Ed Sheeran',       artwork_url: null, preview_url: null },
  { section: 'Last Dance',   track_id: '1440830607', track_name: "Don't Stop Believin'", artist_name: 'Journey',         artwork_url: null, preview_url: null },
];
  ...
  try {
    ...
    for (const v of VENDORS) {
      await client.query(
        ...
        [userId, v.name, v.category, v.status, v.phone, v.email, v.website, v.notes],
      );
      log(`${v.name} (${v.status})`);
    }
    ...
    for (const m of MUSIC) {
      const { rows } = await client.query(
        `INSERT INTO music_tracks
         ...
         RETURNING id`,
        [userId, m.section, m.track_id, m.track_name, m.artist_name, m.artwork_url, m.preview_url],
      );
