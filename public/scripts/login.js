...
function bannerError(prefix, msg) {
  ...
  if (banner && span) { span.textContent = msg; banner.hidden = false; }
}
...
loginForm.addEventListener('submit', async (e) => {
  ...
  if (!email)    { fieldError('login-email-error', 'Email is required');    valid = false; }
  if (!password) { fieldError('login-password-error', 'Password is required'); valid = false; }
  if (!valid) return;
...
regForm.addEventListener('submit', async (e) => {
  ...
  if (!email)     { fieldError('reg-email-error', 'Email is required');    valid = false; }
  if (!password)  { fieldError('reg-password-error', 'Password is required'); valid = false; }
  ...
  try {
    await Auth.register({ email, password, displayName: displayName || undefined });
    window.location.replace('/index.html');
  } catch (err) {
    setLoading(btn, false);
    if (err instanceof ApiResponseError) {
      if (err.status === 409) {
        fieldError('reg-email-error', 'An account with that email already exists');
        ...
        bannerError('reg', err.message || 'Registration failed. Please try again.');
      }
    } else {
