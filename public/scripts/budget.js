...
import { initNav, Toast, formatCurrency, escapeHtml,
         showLoading, showError }                       from './main.js';
...
class BudgetManager {
  ...
  async _loadItems() {
    const query = this._activeFilter !== 'All' ? { category: this._activeFilter } : {};
  ...
  async _addExpense() {
    ...
    if (isNaN(amount) || amount <= 0) return Toast.show('Please enter a valid amount.', 'error');
  ...
  async _saveLimit() {
    ...
    if (isNaN(val) || val < 0) return Toast.show('Enter a valid budget amount.', 'error');
